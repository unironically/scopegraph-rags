...
let rec pattern_contains_var (s: string) (p: pattern) (acc: bool): bool =
  acc ||
  match p with
  | StringP(_) -> false
  | UnderscoreP -> false
  | VarP(s') -> s = s'
  | NilP -> false
  | ConsP(hp, tp) -> (pattern_contains_var s hp false) || (pattern_contains_var s tp false)
  | TupleP(ps) ->   List.fold_right (pattern_contains_var s) ps false
  | TermP(_, ps) -> List.fold_right (pattern_contains_var s) ps false
...
let rec substitute_expr (s: string) (e': expr) (e: expr): expr =
  let substitute_case ((p, e): case): case =
    if pattern_contains_var s p false
    then (p, e)
    else (p, substitute_expr s e' e)
  in
